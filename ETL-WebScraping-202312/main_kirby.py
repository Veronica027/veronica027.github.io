...
def schedule_task():
    ...
    for account in account_list:
        ...
        for q in range(4):
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM matched_data WHERE Status IS NULL LIMIT 200")
            data_result = cursor.fetchall()
            cursor.close()
            cursor = conn.cursor()
            cursor.execute("UPDATE matched_data SET Status = 'completed', Account = %s, completion_time = %s WHERE Status IS NULL LIMIT 200",(account,datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
            conn.commit()
            cursor.close()
            ...
            conn.close()
            time.sleep((q+1)*60*60)


#In[]

if __name__ == '__main__':
    schedule.every().day.at("06:00").do(schedule_task)
    while True:
