...
def task():
    ...
    for i in range(2):
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT company, filter_url FROM company_filter WHERE Account = 'ella' AND Status IS NULL LIMIT 1")
        result = cursor.fetchall()
        cursor.close()
        conn.close()
        ...
        if final == 200:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE company_filter SET Status = 'done', compeletion_time = %s WHERE Account = 'ella' AND Status IS NULL LIMIT 1", (datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
            conn.commit()
            cursor.close()
            conn.close()
        time.sleep(60*60*4)
