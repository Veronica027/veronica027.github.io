...
def schedule_task():
    ...
    for account in account_list:
        ...
        if account != 'shuang' and account != 'kirby':
            cursor.execute("SELECT * FROM matched_data_remaining WHERE Status IS NULL LIMIT 200")
            data_result = cursor.fetchall()
            cursor.close()
            cursor = conn.cursor()
            cursor.execute("UPDATE matched_data_remaining SET Status = 'completed', Account = %s, completion_time = %s WHERE Status IS NULL LIMIT 200",(account,datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
            conn.commit()
            cursor.close()
        else:
            #for test use !!!!!
            cursor.execute("SELECT * FROM matched_data_remaining WHERE Status IS NULL LIMIT 300")
            data_result = cursor.fetchall()
            cursor.close()
            cursor = conn.cursor()
            cursor.execute("UPDATE matched_data_remaining SET Status = 'completed', Account = %s, completion_time = %s WHERE Status IS NULL LIMIT 300",(account,datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
            conn.commit()
            cursor.close()
