...
def schedule_task():
    ...
    for account in account_list:
        ...
        for q in range(8):
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM new_6com_talents WHERE Status = 'wait' AND Account != 'kirby' LIMIT 130")
            data_result = cursor.fetchall()
            cursor.close()
            cursor = conn.cursor()
            cursor.execute("UPDATE new_6com_talents SET Status = 'completed', Account = %s, completion_time = %s WHERE Status = 'wait' AND Account != 'kirby' LIMIT 130",(account,datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
            conn.commit()
            cursor.close()
