...
def profileupdate():
    ...
    thread = threading.Thread(target= updatedetail, args = (driver_2,talent_link,username,password,public_link,name,linkedin_account[0]))
    thread.start()
    return jsonify({"status":200,"data":"success"})



def updatedetail(driver_2,talent_link,username,password,public_link,name,account):
    saved_data = {}
    talent_not_valid = {}
    api_not_work = []
    quota_exceed = []
    ...
    for i in range(len(talent_link)):
        result_data = sourcing_pipeline(driver_2,talent_link[i],username,password,i,"https://www.linkedin.com/talent/profile/ACoAACeOyc0BKG-njJACAXhvTR2-oWhu0MzyAjU")
        if result_data == 100:
            talent_not_valid[name[i]] = public_link[i]
        elif result_data == 404:
            api_not_work.append(public_link[i])
        ...
        if i%100 == 0 and i != 0:
            ...
            key = "buffer-files-update/raw-" + account + \
            ...
            key_2 = "buffer-files-update/invalid-" + account + \
                "-" + "update" + "-" + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ".pkl"
    ...
    cursor = conn.cursor()
    for i in range(len(api_not_work)):
        public_link = api_not_work[i]
        cursor.execute("UPDATE matched_data SET Status = 'API INVALID' WHERE linkedin = %s", (public_link,))
        conn.commit()
...
def sourcing_pipeline(driver, talent_link,username,password,num,alternative_talent_link):
    if num == 0:
    ...
    # try:
    #     try:
    #         driver.driver.find_element(by=By.ID, value='username').clear()
    #         driver.find_element(by=By.ID, value='username').send_keys(username)
    #         driver.find_element(by=By.ID, value='password').send_keys(password)
    #         driver.find_element(By.XPATH,"//div[contains(@class,'login__form_action_container')]//button").click()
    #         try:
